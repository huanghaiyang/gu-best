...
class EastmoneyAPI(StockDataProvider):
        ...
        while retry_count < max_retries:
            try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(url, params=params, timeout=self.LONG_TIMEOUT) as response:
                        # 检查响应状态码
                        if response.status != 200:
                            print(f"获取所有股票失败: 状态码 {response.status}")
                            retry_count += 1
                            continue
    ...
    async def _make_request(self, url: str, params: Dict, timeout: int = None) -> Optional[Dict]:
        """发送HTTP请求并返回JSON数据"""
        if timeout is None:
